 ...
 * 查询规划图 场景测试
 * 测试各种树结构下的查询规划图视图输出
 * 场景迁移自 StrategyLoops/loops/plan-exe-eval/test/规划图Prompt测试.ts
 */
...
const TEST_PROJECT_DIR = join(__dirname, "data", ".scheduleMap.scenario")
const TEST_DB_PATH = join(TEST_PROJECT_DIR, "scenarioScheduleMap.db")
...
describe("场景1: 多根任务", () => {
  beforeAll(() => {
    clearAllTasks()
    规划图.添加任务(null, "项目A总体规划", "项目A", 0, 任务Tag.MILESTONE)
    规划图.添加任务("项目A", "功能1", "项目A-功能1", 0, 任务Tag.FEAT)
    规划图.添加任务("项目A", "功能2", "项目A-功能2", 1, 任务Tag.FEAT)
    规划图.添加任务(null, "项目B总体规划", "项目B", 1, 任务Tag.MILESTONE)
    规划图.添加任务("项目B", "功能1", "项目B-功能1", 0, 任务Tag.FEAT)
    规划图.添加任务(null, "项目C总体规划", "项目C", 2, 任务Tag.MILESTONE)
  })
  ...
  test("视图包含所有根任务 - 按优先级排序", () => {
    const view = 查询规划图_返回视图(30, undefined, undefined, 100, 100)
...
describe("场景2: 深层嵌套（深度=10）", () => {
  beforeAll(() => {
    clearAllTasks()
    规划图.添加任务(null, "第0层", "L0-任务", 0, 任务Tag.FEAT)
    for (let i = 1; i <= 10; i++) {
      规划图.添加任务(`L${i - 1}-任务`, `第${i}层`, `L${i}-任务`, 0, 任务Tag.FEAT)
    }
  })
...
describe("场景3: 宽分支（单个父任务有10个子任务）", () => {
  beforeAll(() => {
    clearAllTasks()
    规划图.添加任务(null, "根", "根任务", 0, 任务Tag.MILESTONE)
    for (let i = 0; i < 10; i++) {
      规划图.添加任务("根任务", `第${i}个子任务`, `子任务-${i}`, i, 任务Tag.FEAT)
    }
  })
...
describe("场景4: 稀疏树（不同根不同深度）", () => {
  beforeAll(() => {
    clearAllTasks()
    规划图.添加任务(null, "项目1", "项目1", 0, 任务Tag.MILESTONE)
    规划图.添加任务("项目1", "功能", "项目1-功能", 0, 任务Tag.FEAT)

    规划图.添加任务(null, "项目2", "项目2", 1, 任务Tag.MILESTONE)
    规划图.添加任务("项目2", "功能", "项目2-功能", 0, 任务Tag.FEAT)
    规划图.添加任务("项目2-功能", "子功能", "项目2-功能-子功能", 0, 任务Tag.FEAT)

    规划图.添加任务(null, "项目3", "项目3", 2, 任务Tag.MILESTONE)
    规划图.添加任务("项目3", "功能", "项目3-功能", 0, 任务Tag.FEAT)
    规划图.添加任务("项目3-功能", "子功能", "项目3-功能-子功能", 0, 任务Tag.FEAT)
    规划图.添加任务("项目3-功能-子功能", "孙功能", "项目3-功能-子功能-孙功能", 0, 任务Tag.FEAT)
  })
...
describe("场景5: 不同路径相同优先级序号", () => {
  beforeAll(() => {
    clearAllTasks()
    规划图.添加任务(null, "项目", "项目", 0, 任务Tag.MILESTONE)

    规划图.添加任务("项目", "分支A", "分支A", 0, 任务Tag.FEAT)
    规划图.添加任务("分支A", "A的任务0", "分支A-任务0", 0, 任务Tag.FEAT)
    规划图.添加任务("分支A", "A的任务1", "分支A-任务1", 1, 任务Tag.FEAT)

    规划图.添加任务("项目", "分支B", "分支B", 1, 任务Tag.FEAT)
    规划图.添加任务("分支B", "B的任务0", "分支B-任务0", 0, 任务Tag.FEAT)
    规划图.添加任务("分支B", "B的任务1", "分支B-任务1", 1, 任务Tag.FEAT)
  })
...
describe("场景6: 超过聚焦数量上限（复杂树结构）", () => {
  beforeAll(() => {
    ...
    // 4个根项目
    规划图.添加任务(null, "项目A总体规划", "项目A-总规划", 0, 任务Tag.MILESTONE)
    规划图.添加任务(null, "项目B总体规划", "项目B-总规划", 1, 任务Tag.MILESTONE)
    规划图.添加任务(null, "项目C总体规划", "项目C-总规划", 2, 任务Tag.MILESTONE)
    规划图.添加任务(null, "项目D总体规划", "项目D-总规划", 3, 任务Tag.MILESTONE)

    // 项目A：2个一级子任务 + 各10个二级子任务
    for (let i = 0; i < 2; i++) {
      规划图.添加任务("项目A-总规划", `A分支${i}`, `A-分支${i}`, i, 任务Tag.FEAT)
      for (let j = 0; j < 10; j++) {
        规划图.添加任务(`A-分支${i}`, `A${i}任务${j}`, `A-分支${i}-任务${j}`, j, 任务Tag.FEAT)
      }
    }

    // 项目B：1个一级子任务 + 20个二级子任务
    规划图.添加任务("项目B-总规划", "B分支0", "B-分支0", 0, 任务Tag.FEAT)
    for (let j = 0; j < 20; j++) {
      规划图.添加任务("B-分支0", `B0任务${j}`, `B-分支0-任务${j}`, j, 任务Tag.FEAT)
    }

    // 项目C：3个一级子任务 + 各15个二级子任务（部分已完成）
    for (let i = 0; i < 3; i++) {
      规划图.添加任务("项目C-总规划", `C分支${i}`, `C-分支${i}`, i, 任务Tag.FEAT)
      for (let j = 0; j < 15; j++) {
        规划图.添加任务(`C-分支${i}`, `C${i}任务${j}`, `C-分支${i}-任务${j}`, j, 任务Tag.FEAT)
      }
    }

    // 项目D：5个一级子任务 + 各10个二级子任务
    for (let i = 0; i < 5; i++) {
      规划图.添加任务("项目D-总规划", `D分支${i}`, `D-分支${i}`, i, 任务Tag.FEAT)
      for (let j = 0; j < 10; j++) {
        规划图.添加任务(`D-分支${i}`, `D${i}任务${j}`, `D-分支${i}-任务${j}`, j, 任务Tag.FEAT)
      }
    }
  })
  ...
  test("聚焦数量为0时返回错误", () => {
    const view = 查询规划图_返回视图(0, undefined, undefined, 100, 100)
    expect(view).toContain("错误")
  })

  test("聚焦数量更小时展示更少的任务", () => {
    const view10 = 查询规划图_返回视图(10, undefined, undefined, 100, 100)
    const view30 = 查询规划图_返回视图(30, undefined, undefined, 100, 100)
