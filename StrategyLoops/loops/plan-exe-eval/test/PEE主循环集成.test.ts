...
import { AbortError, INTERRUPTION_REASON, MSG_SOURCE, type InterruptedMsgContext } from "../../../common/types"
...
class ScriptedSession implements ISession {
  ...
  role: IRole
  directory: string
  private messages: { msgSource: string; content: string }[] = []
  ...
  async getMessages(): Promise<{ msgSource: string; content: string }[]> { return this.messages }
  getTokenUsage() { return undefined }

  async sendMsg(message: { msgSource: string; content: string }): Promise<string> {
  ...
  async function runMainWithScript(options: {
    ...
    try {
      ...
      if (options.interruptRoleName) {
        const interruptedSession = await createSession.mock.results[0]?.value
        ;(interruptedSession as ScriptedSession | undefined)?.emitInterruption({
