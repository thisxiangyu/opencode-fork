...
export const defaultRalphLoopStrategy: LoopStrategy = {
  ...
  transitions: [
    createTransition({ from: "expert", to: "plan", condition: { type: "always" }, description: "专家意见完成后进入规划" }),
    createTransition({ from: "plan", to: "execute", condition: { type: "always" }, description: "规划完成后进入执行" }),
    createTransition({ from: "execute", to: "analyze", condition: { type: "always" }, description: "执行完成后进入测试分析" }),
    ...
    createTransition({ from: "review", to: "optimize", condition: { type: "always" }, description: "审核完成后进入优化" }),
    createTransition({ from: "optimize", to: "quality", condition: { type: "always" }, description: "优化完成后进入质量评估" }),
    createConditionalTransition({ from: "quality", to: "commit", field: "qualityPassed", value: true, conditionType: "equals", description: "质量评估通过", priority: 1 }),
    createConditionalTransition({ from: "quality", to: "plan", field: "qualityPassed", value: false, conditionType: "equals", description: "质量评估失败，重新规划", priority: 2 }),
    createConditionalTransition({ from: "commit", to: "exit", field: "shouldExit", value: true, conditionType: "equals", description: "满足退出条件", priority: 3 }),
    createConditionalTransition({ from: "commit", to: "expert", field: "ueApproved", value: true, conditionType: "equals", description: "用户体验通过，回到专家意见(进行下一轮)", priority: 2 }),
    createConditionalTransition({ from: "commit", to: "plan", field: "ueApproved", value: false, conditionType: "equals", description: "用户体验不通过，重新规划", priority: 1 }),
  ],
}
