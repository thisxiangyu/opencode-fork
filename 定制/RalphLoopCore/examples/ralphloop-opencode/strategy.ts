...
function createNode(
  ...
  return createLoopNode(
    id,
    name,
    [createRoleInst(roleId, roleName, systemPrompt)],
...
const analyzeNode = createNode(
  ...
  "资深分析师",
  buildRolePrompt(
    "资深的分析师，擅长从多个角度发现潜在问题",
    "从N个角度分析可能出现的问题：大数据量情况、边缘条件、边缘逻辑、整体架构视角、被依赖文件、未来破坏性变更影响等等。汇报并列出必须要测试的点。",
    [
      "大数据量情况分析",
      "边缘条件分析",
      "边缘逻辑分析",
...
const testNode = createNode(
  ...
  buildRolePrompt(
    ...
    "根据分析节点的结果编写N个角度的测试用例，并执行测试",
    ["根据分析结果编写测试用例", "执行测试并记录结果", "提供测试覆盖率报告"],
    "编写测试",
...
const verifyNode = createNode(
  ...
  "代码校验专家",
  buildRolePrompt(
    "代码校验专家，擅长代码优雅性和最佳实践检查",
    "根据分析、测试、审核的结果进行批量纠偏：逐个检查是否采用最佳实践、N个代码优雅的标准、N个性能优化点、是否拆动墙补西墙、不要把简单的事情搞复杂（如许多地方nameof()比string字面量要好）",
    [
      "检查是否采用最佳实践",
      "检查代码优雅标准",
      "检查性能优化点",
...
export const ralphLoopStrategy: LoopStrategy = {
  ...
  transitions: [
    createTransition("opinion", "plan", { type: "always" }, "意见完成后进入规划"),
    createTransition("plan", "execute", { type: "always" }, "规划完成后进入执行"),
    createTransition("execute", "analyze", { type: "always" }, "执行完成后进入分析"),
    ...
    createTransition("review", "verify", { type: "always" }, "审核完成后进入校验"),
    createTransition("verify", "finalEval", { type: "always" }, "校验完成后进入终评"),
    createConditionalTransition("finalEval", "experience", "qualityPassed", true, "equals", "终评通过", 1),
    createConditionalTransition("finalEval", "plan", "qualityPassed", false, "equals", "终评不通过，重新规划", 2),
    createConditionalTransition("experience", "commit", "ueApproved", true, "equals", "体验通过，提交代码", 1),
    createConditionalTransition("experience", "plan", "ueApproved", false, "equals", "体验不通过，重新规划", 2),
    createTransition("commit", "opinion", { type: "always" }, "提交后回到意见节点进行下一轮"),
    createTransition("commit", "exit", { type: "always" }, "提交完成正常结束"),
  ],
}
