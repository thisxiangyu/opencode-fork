...
// 定制：worktree 路径支持通过 OPENCODE_WORKTREE 环境变量或全局配置的 storage.worktree 覆盖
export function resolveDir(projectID?: string): string {
  const base = StorageConfig.resolvePath({
    type: "worktree",
    flag: Flag.OPENCODE_WORKTREE,
    defaultPath: nodePath.join(Global.Path.data, "worktree"),
    allowRelative: true,
  })
  return projectID ? nodePath.join(base, projectID) : base
}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const makeWorktreeInfo = Effect.fn("Worktree.makeWorktreeInfo")(function* (input?: {
      ...
      const root = pathSvc.join(resolveDir(ctx.project.id))
