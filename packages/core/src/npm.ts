...
export interface Interface {
  ...
  // 定制（Windows 兼容性修复）：install=false 时仅查询缓存不触发 npm install，
  // 避免网络受限环境下自动下载阻塞（如写入 .sh 触发 bash-language-server 安装挂起）
  readonly which: (pkg: string, bin?: string, install?: boolean) => Effect.Effect<string | undefined>
}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const which = Effect.fn("Npm.which")(function* (pkg: string, bin?: string, install = true) {
      const dir = directory(pkg)
      ...
      return Option.getOrUndefined(
        yield* Effect.gen(function* () {
          ...
          if (!install) return Option.none<string>()
