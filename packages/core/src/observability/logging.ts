...
import { resolveLogDir } from "../util/log"
...
// 定制：日志目录支持通过 OPENCODE_LOG 环境变量或全局配置的 storage.log 覆盖（见 util/log.ts 的 resolveLogDir）
export function fileLogger(file = path.join(resolveLogDir(), "opencode.log"), id: string = runID) {
