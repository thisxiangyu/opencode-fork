...
export const Info = Schema.Struct({
  ...
  // 定制：可配置存储路径（数据库、日志、worktree、snapshot），由 core/storage/storage-config.ts 解析
  storage: Schema.optional(
    Schema.Struct({
      database: Schema.optional(Schema.String).annotate({ description: "Custom path for database file" }),
      log: Schema.optional(Schema.String).annotate({ description: "Custom path for log directory" }),
      worktree: Schema.optional(Schema.String).annotate({ description: "Custom path for worktree directory" }),
      snapshot: Schema.optional(Schema.String).annotate({ description: "Custom path for snapshot directory" }),
    }),
  ).annotate({ description: "Custom storage paths for database, logs, worktrees, and snapshots" }),
