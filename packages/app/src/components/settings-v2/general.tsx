...
export const SettingsGeneralV2: Component<{
  ...
  // 定制：存储与全局配置路径展示（对齐旧版 settings-general.tsx 底部区块）
  const [storageStore, setStorageStore] = createStore({
  ...
  const loadGlobalConfigPath = async () => {
    try {
      const ctx = storageHeaders()
      if (!ctx) return
  ...
  const initGlobalConfig = async () => {
    try {
      ...
      setStorageStore("globalConfigPath", data.path)
      setStorageStore("globalConfig", { $schema: "https://opencode.ai/config.json" })
    } catch {}
  }
  ...
  const loadWorktreePath = async () => {
    try {
      const ctx = storageHeaders()
      if (!ctx) return
      const res = await fetch(`${ctx.url}/global/storage/worktree`, { headers: ctx.headers })
      if (res.ok) {
        const data = await res.json()
        setStorageStore("worktreePath", data.path ?? "")
      }
    } catch {
      setStorageStore("worktreePath", "")
    }
  }

  const loadSnapshotPath = async () => {
    try {
      const ctx = storageHeaders()
      if (!ctx) return
      const res = await fetch(`${ctx.url}/global/storage/snapshot`, { headers: ctx.headers })
      if (res.ok) {
        const data = await res.json()
        setStorageStore("snapshotPath", data.path ?? "")
      }
    } catch {
      setStorageStore("snapshotPath", "")
    }
  }

  const copyPath = (text: string, setCopied: (value: boolean) => void) => {
  ...
  onMount(() => {
    ...
    void loadGlobalConfigPath()
    void loadWorktreePath()
    void loadSnapshotPath()
  })
  ...
  const StoragePathRow = (rowProps: {
    ...
    >
      <button
        ...
        onClick={rowProps.onCopy}
        title={rowProps.copyTitle}
      >
        <Icon
          name={rowProps.copied ? "check" : "copy"}
          size="small"
          class={rowProps.copied ? "text-icon-success-base" : "text-icon-base"}
        />
      </button>
    </SettingsRowV2>
  )
  ...
  const WorktreeSection = () => (
    <Show when={storageStore.worktreePath}>
      <div class="settings-v2-section">
        <h3 class="settings-v2-section-title">{language.t("settings.general.section.worktree")}</h3>

        <SettingsListV2>
          <StoragePathRow
            title={language.t("settings.general.row.worktree.path")}
            path={storageStore.worktreePath}
            copied={worktreeCopied()}
            copyTitle={language.t("settings.general.row.worktree.copy")}
            onCopy={() => copyPath(storageStore.worktreePath, setWorktreeCopied)}
          />
        </SettingsListV2>
      </div>
    </Show>
  ...
  const SnapshotSection = () => (
    <Show when={storageStore.snapshotPath}>
      <div class="settings-v2-section">
        <h3 class="settings-v2-section-title">{language.t("settings.general.section.snapshot")}</h3>

        <SettingsListV2>
          <StoragePathRow
            title={language.t("settings.general.row.snapshot.path")}
            path={storageStore.snapshotPath}
            copied={snapshotCopied()}
            copyTitle={language.t("settings.general.row.snapshot.copy")}
            onCopy={() => copyPath(storageStore.snapshotPath, setSnapshotCopied)}
          />
        </SettingsListV2>
      </div>
    </Show>
  ...
  // 定制：全局配置文件路径展示；不存在时提供创建按钮
  const ConfigFileSection = () => (
    <div class="settings-v2-section">
      <h3 class="settings-v2-section-title">{language.t("settings.data.section.configFile")}</h3>

      <SettingsListV2>
        <Show when={storageStore.globalConfigPath}>
          <StoragePathRow
            title={language.t("settings.data.row.configFile.globalConfig")}
            path={storageStore.globalConfigPath}
            copied={configCopied()}
            copyTitle={language.t("settings.data.row.configFile.copy")}
            onCopy={() => copyPath(storageStore.globalConfigPath, setConfigCopied)}
          />
        </Show>

        <Show when={!storageStore.globalConfigPath && Object.keys(storageStore.globalConfig).length === 0}>
          <SettingsRowV2
            title={language.t("settings.data.row.configFile.globalConfig")}
            description={language.t("settings.data.row.configFile.noConfig")}
          >
            <ButtonV2 size="normal" variant="neutral" onClick={initGlobalConfig}>
